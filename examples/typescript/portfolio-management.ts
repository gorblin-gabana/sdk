...
async function advancedPortfolioExample() {
  ...
  for (const [type, address] of Object.entries(wallets)) {
    ...
    try {
      ...
      console.log(
        `     Unique Programs: ${portfolio.diversification.programCount}`,
      );
      console.log(
        `     Concentration Risk: ${(Number(portfolio.diversification.concentrationRisk) * 100).toFixed(1)}%`,
      );

      const riskLevel =
        Number(portfolio.diversification.concentrationRisk) > 0.7
          ? "High"
          : Number(portfolio.diversification.concentrationRisk) > 0.4
            ? "Medium"
            : "Low";
      ...
      console.log("\n   🏷️ Token Breakdown:");
      console.log(
        `     Fungible Tokens: ${portfolio.tokenTypes.fungibleTokens}`,
      );
      ...
      if (topHoldings.length > 0) {
        ...
        topHoldings.forEach((holding, index) => {
          console.log(
            `     ${index + 1}. ${holding.metadata?.symbol || "Unknown"}`,
          );
          console.log(
            `        Balance: ${holding.balance?.formatted || holding.balance}`,
          );
          console.log(
            `        Program: ${holding.programId?.substring(0, 8)}...`,
          );
        });
      }
  ...
  try {
    const comparison = await sdk.comparePortfolios(
      wallets.defiTrader,
      wallets.nftCollector,
    );

    console.log("📊 Comparison Results:");
    console.log(`   Common Tokens: ${comparison.commonTokens.length}`);
    console.log(
      `   Unique to DeFi Trader: ${comparison.uniqueToWallet1.length}`,
    );
    console.log(
      `   Unique to NFT Collector: ${comparison.uniqueToWallet2.length}`,
    );
    console.log(
      `   Portfolio Similarity: ${(comparison.similarity * 100).toFixed(1)}%`,
    );
    ...
    if (comparison.commonTokens.length > 0) {
      ...
      comparison.commonTokens.slice(0, 3).forEach((token, index) => {
        console.log(
          `     ${index + 1}. ${token.metadata?.symbol || "Unknown Token"}`,
        );
      });
    }
  } catch (error) {
    console.log(
      "❌ Portfolio comparison failed (expected with example addresses)",
    );
  }
  ...
  console.log("```typescript");
  console.log(
    "const suggestions = await generateRebalancingSuggestions(portfolio);",
  );
...
function generateRebalancingSuggestions(portfolio: any) {
  ...
  if (
    portfolio.tokenTypes.nfts === 0 &&
    portfolio.tokenTypes.fungibleTokens > 5
  ) {
    suggestions.push({
...
const isMainModule =
  process.argv[1] && process.argv[1].endsWith("portfolio-management.ts");
if (isMainModule) {
