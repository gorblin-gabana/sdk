...
async function richTransactionAnalysisExample() {
  ...
  for (const [index, txExample] of exampleTransactions.entries()) {
    ...
    try {
      ...
      richTx.instructions.forEach((instruction, i) => {
        ...
        console.log(
          `        Success: ${instruction.result.success ? "✅" : "❌"}`,
        );

        // Token transfer details
        if (
          instruction.tokens?.transfers &&
          instruction.tokens.transfers.length > 0
        ) {
          console.log("        Token Transfers:");
          instruction.tokens.transfers.forEach((transfer) => {
            console.log(
              `          • ${transfer.amountFormatted} ${transfer.token.symbol || "TOKEN"}`,
            );
            console.log(
        ...
        if (instruction.solChanges && instruction.solChanges.length > 0) {
          ...
          instruction.solChanges.forEach((change) => {
            console.log(
              `          • ${change.type === "credit" ? "+" : "-"}${Math.abs(change.changeSOL)} SOL`,
            );
            console.log(
              `            Account: ${change.account.substring(0, 8)}...`,
            );
          });
        }
      });
      ...
      if (richTx.balanceChanges.sol.length > 0) {
        ...
        richTx.balanceChanges.sol.forEach((change) => {
          ...
          console.log(
            `       ${change.account.substring(0, 8)}...: ${changeSOL > 0 ? "+" : ""}${changeSOL.toFixed(6)} SOL`,
          );
        });
      }
      ...
      if (richTx.balanceChanges.tokens.length > 0) {
        ...
        richTx.balanceChanges.tokens.forEach((change) => {
          console.log(
            `       ${change.account.substring(0, 8)}...: ${change.changeFormatted} ${change.token.symbol || "TOKEN"}`,
          );
        });
      }
      ...
      richTx.summary.participants.slice(0, 3).forEach((participant) => {
        console.log(
          `     ${participant.role}: ${participant.address.substring(0, 8)}...`,
        );
      ...
      console.log(
        `     Programs Involved: ${richTx.meta.programsInvolved.length}`,
      );
      console.log(
        `     Metadata Resolved: ${richTx.meta.metadataResolved ? "✅" : "❌"}`,
      );
    } catch (error) {
      console.log(
  ...
  console.log(
    'import { getRichTransaction } from "@gorbchain-xyz/chaindecode";',
  );
  console.log("");
...
const isMainModule =
  process.argv[1] && process.argv[1].endsWith("transaction-analysis.ts");
if (isMainModule) {
