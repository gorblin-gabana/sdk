...
const RichTransactionDisplay: React.FC<RichTransactionDisplayProps> = ({
  transaction,
  sdk,
}) => {
  const [enhancedInstructions, setEnhancedInstructions] = React.useState<
    EnhancedInstruction[]
  >([]);
    ...
    try {
      ...
      for (const instruction of instructions) {
        ...
        let enhancedInst: EnhancedInstruction = {
          ...
          programId,
          accounts:
            instruction.accounts?.map((idx: number) => accountKeys[idx]) || [],
        };
  ...
  const decodeToken2022Instruction = async (
    data: number[],
  ): Promise<{
    type: string;
    instruction: string;
  ...
  const decodeSPLTokenInstruction = async (
    data: number[],
  ): Promise<{
    type: string;
    instruction: string;
  ...
  const decodeSystemInstruction = async (
    data: number[],
  ): Promise<{
    type: string;
    instruction: string;
    ...
    switch (instructionType) {
      case 0: // CreateAccount
        if (data.length >= 49) {
          ...
          const amount = new DataView(
            new Uint8Array(lamportsArray).buffer,
          ).getBigUint64(0, true);
          return {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <p className="font-mono text-sm">
              {transaction.transaction?.signatures?.[0]?.slice(0, 8)}...
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm">
              {formatAmount(BigInt(Number(transaction.meta?.fee || 0)), 9)} SOL
            </p>
          </div>
          <div>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4
                      className={`font-medium ${getInstructionColor(inst.type)}`}
                    >
                      {inst.instruction}
                    </h4>
                    <div className="flex items-center space-x-2 mt-1">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${getBadgeColor(inst.programName)}`}
                      >
                        {inst.programName}
                      </span>
                  ...
                  <div className="mt-1 space-y-1">
                    ...
                    {inst.accounts.length > 3 && (
                      <div className="text-gray-400">
                        ... and {inst.accounts.length - 3} more
                      </div>
                    )}
                  </div>
                </div>
