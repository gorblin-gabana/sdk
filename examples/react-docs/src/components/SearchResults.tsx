...
const SearchResults: React.FC = () => {
  ...
  }, [isSearchOpen, searchResults, selectedIndex, setSelectedIndex, navigateToResult])
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'page': return <DocumentTextIcon className="h-4 w-4" />
      case 'code': return <CodeBracketIcon className="h-4 w-4" />
      case 'section': return <DocumentTextIcon className="h-4 w-4" />
      case 'config': return <Cog6ToothIcon className="h-4 w-4" />
      case 'error': return <ExclamationTriangleIcon className="h-4 w-4" />
      default: return <RocketLaunchIcon className="h-4 w-4" />
    }
  }
  ...
  const highlightMatch = (text: string, query: string) => {
    ...
    return parts.map((part, index) => {
      if (part.toLowerCase() === query.toLowerCase()) {
        return <span key={index} className="bg-gorbchain-highlight text-gorbchain-secondary px-0.5 rounded font-medium">{part}</span>
      }
      return part
    })
  }
    ...
    >
      {searchResults.length === 0 ? (
        <div className="p-4 text-center text-gray-500">
          ...
          <p className="text-sm mt-1">Try searching for: api, rpc, error, decode, setup</p>
        </div>
      ) : (
        <div className="py-2">
          ...
          {searchResults.map((result, index) => (
            ...
            >
              <div className="flex items-start space-x-3">
                <div className={`flex-shrink-0 p-1.5 rounded-md ${getTypeColor(result.type)}`}>
                  {getIcon(result.type)}
                </div>
