...
export class MethodExecutor {
  ...
  async executeMethod(method: SDKMethod, params: { [key: string]: string }): Promise<any> {
    if (!this.sdk) {
    ...
    try {
      ...
      switch (method.name) {
        ...
        case 'getAndDecodeTransaction':
          const signature = params.signature || '5Nm3CvXWYjDaeVPTXifXHFzpovVZo6pLQdMfZeW5FoUK9z3vE4ABM'
          let options: any = {}
        ...
        case 'createToken22TwoTx':
          ...
          if (params.params) {
            try {
              ...
              tokenParams = { name: 'Test Token', symbol: 'TEST', decimals: 6, supply: 1000000 }
            }
          }
          
          result = {
            success: true,
            message: 'Demo mode - actual token creation requires wallet connection',
        ...
        case 'createNFT':
          ...
          if (params.params) {
            try {
              ...
              console.warn('Invalid JSON in params parameter, using default')
              nftParams = { name: 'Test NFT', symbol: 'TESTNFT', uri: 'https://example.com/metadata.json' }
            }
          }
          
          result = {
            success: true,
            message: 'Demo mode - actual NFT creation requires wallet connection',
            note: 'In production, pass a wallet adapter to create actual NFTs',
            demoData: {
              mint: 'DemoNFT' + Math.random().toString(36).substr(2, 9),
              metadataAccount: 'DemoMeta' + Math.random().toString(36).substr(2, 9),
              estimatedCost: 0.005,
