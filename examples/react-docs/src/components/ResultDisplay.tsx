...
import { CheckCircleIcon, XCircleIcon, ClipboardIcon, CheckIcon } from '@heroicons/react/24/outline'
...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, methodName }) => {
      ...
      <div className={`rounded-lg p-4 ${isError ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
        ...
        {!isError && (
          <div className="space-y-2">
            ...
            {methodName === 'getAndDecodeTransaction' && result?.transactionType && (
              <div className="space-y-2">
                ...
                {result.analysis && (
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2">{result.analysis.totalInstructions}</span>
                    </div>
                    <div>
                      <span className="font-medium">Compute Units:</span>
                      <span className="ml-2">{result.analysis.totalComputeUnits}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {result?.tokenInfo && (
              <div className="bg-blue-50 border border-blue-200 rounded p-3">
                <div className="text-sm font-medium text-blue-800 mb-2">Token Information</div>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="font-medium">Operations:</span>
                    <span className="ml-2">{result.tokenInfo.totalOperations}</span>
                  </div>
                  <div>
                    <span className="font-medium">Accounts:</span>
                    <span className="ml-2">{result.tokenInfo.totalAccounts}</span>
                  </div>
                </div>
              </div>
            )}
