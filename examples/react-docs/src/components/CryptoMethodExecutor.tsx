...
export function CryptoMethodExecutor({ method, generatedKeys = [] }: Props) {
  ...
  const fillWithGeneratedKey = (paramName: string, keyType: 'publicKey' | 'privateKey', keyIndex: number = 0) => {
    if (generatedKeys[keyIndex]) {
      setParams(prev => ({ 
        ...prev, 
        [paramName]: keyType === 'privateKey' 
      ...
      switch (method.id) {
        ...
        case 'personal-encrypt':
          ...
          result = {
            success: true,
            demoMode: true,
            encryptedData: 'demo_encrypted_' + Buffer.from(params.data).toString('base64'),
            method: 'personal',
            metadata: {
        ...
        case 'personal-decrypt':
          ...
          if (params.encryptedResult.includes('demo_encrypted_')) {
            const demoData = params.encryptedResult.replace('demo_encrypted_', '')
            result = {
              success: true,
              demoMode: true,
        ...
        case 'direct-encrypt':
          if (!params.data || !params.recipientPublicKey || !params.senderPrivateKey) {
            throw new Error('Data, recipient public key, and sender private key are required')
          }

          result = {
            success: true,
            demoMode: true,
            encryptedData: 'demo_direct_encrypted_' + Buffer.from(params.data).toString('base64'),
            method: 'direct',
            metadata: {
        ...
        case 'direct-decrypt':
          if (!params.encryptedResult || !params.recipientPrivateKey) {
            throw new Error('Encrypted result and recipient private key are required')
          }

          if (params.encryptedResult.includes('demo_direct_encrypted_')) {
            const demoData = params.encryptedResult.replace('demo_direct_encrypted_', '')
            result = {
              success: true,
              demoMode: true,
              decryptedData: Buffer.from(demoData, 'base64').toString(),
        ...
        case 'sign-data':
          ...
          result = {
            success: true,
            demoMode: true,
            signature: 'demo_signature_' + Buffer.from(params.data + Date.now()).toString('base64'),
            data: params.data,
            algorithm: 'ed25519',
            timestamp: new Date().toISOString(),
        ...
        case 'verify-signature':
          ...
          const isValidDemo = params.signature?.includes('demo_signature_') && 
                             params.signature?.includes(Buffer.from(params.data).toString('base64'))
        ...
        default:
          result = {
            ...
            availableMethods: [
              'generate-keypair', 'personal-encrypt', 'personal-decrypt',
              'direct-encrypt', 'direct-decrypt', 'create-signature-group',
        ...
        <div className="bg-gray-50 p-6 border-b border-gray-200">
          ...
          <div className="space-y-4">
            {method.params.map((param) => (
              <div key={param.name} className="bg-white rounded-lg p-4 border border-gray-200">
                <div className="mb-3">
                ...
                {(param.name.includes('privateKey') || param.name.includes('publicKey')) && generatedKeys.length > 0 && (
                  <div className="mt-3 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                    <p className="text-xs font-medium text-emerald-800 mb-2">Quick Fill with Generated Keys:</p>
                    <div className="flex flex-wrap gap-2">
                      {generatedKeys.slice(0, 3).map((_, index) => (
                        <button
                          key={index}
                          onClick={() => fillWithGeneratedKey(
                            param.name, 
                            param.name.includes('privateKey') ? 'privateKey' : 'publicKey',
                            index
                          )}
                          className="btn-primary text-xs px-3 py-2 rounded-lg transition-all duration-200"
                        >
                          Key {index + 1} {param.name.includes('privateKey') ? '🔑' : '🔓'}
                        </button>
                      ))}
                    </div>
                  </div>
