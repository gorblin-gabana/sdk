...
function renderValue(value: any, _key: string, depth: number = 0): JSX.Element {
  ...
  if (Array.isArray(value)) {
    ...
    return <CollapsibleArray items={value} depth={depth} maxPreview={3} />;
  }
  ...
  if (typeof value === "object") {
    ...
    return <CollapsibleObject obj={value} depth={depth} maxPreview={3} />;
  }
...
function CollapsibleArray({
  ...
  return (
    <div>
      ...
      <div className="ml-4 space-y-1">
        {items
          .slice(0, expanded ? items.length : maxPreview)
          .map((item, index) => (
            <div key={index}>
    ...
    <div className="bg-gray-50 rounded-lg border">
      ...
      {!collapsed && (
        <div className="p-4 font-mono text-sm overflow-x-auto">
          <div className="space-y-1">{renderValue(data, "root")}</div>
        </div>
      )}
    </div>
  );
