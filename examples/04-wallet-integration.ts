...
async function walletIntegrationExample() {
  ...
  try {
    ...
    console.log(`  Previously Connected: ${discovery.previouslyConnected.length}`);
  ...
  for (const walletType of supportedWallets) {
    try {
      ...
      console.log(`❌ Failed to connect to ${walletType} (expected in Node.js)`);
    }
  }
  ...
  console.log('  const [walletManager] = useState(() => sdk.createWalletManager());');
  console.log('  const [wallet, setWallet] = useState<RichWallet | null>(null);');
  console.log('  ');
...
function displayWalletInfo(wallet: any) {
  ...
  console.log(`  Type: ${wallet.type}`);
  console.log(`  Address: ${wallet.address.substring(0, 8)}...${wallet.address.substring(-8)}`);
  ...
  if (wallet.portfolio.topTokens.length > 0) {
    console.log(`\n🏆 Top Token Holdings:`);
    wallet.portfolio.topTokens.slice(0, 3).forEach((token: any, index: number) => {
