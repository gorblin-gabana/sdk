...
async function main() {
  ...
  console.log('Encrypted data:', encrypted1.encryptedData.substring(0, 50) + '...');
  ...
  console.log('Shared key ID:', updatedContext1.sharedKeyId?.substring(0, 16) + '...');
  console.log();
  ...
  console.log('Created shared key for meetings:', meetingKey.keyId.substring(0, 16) + '...');
...
- Charlie: Prepare documentation
  ...
  const teamMessage = 'Welcome Diana and Eve! You now have access to all project communications.';
  ...
  console.log('Recovered key ID:', recoveredKey.keyId === meetingKey.keyId ? 'MATCH' : 'MISMATCH');
  ...
  const sensitiveMessage = 'This is sensitive information that Charlie should not see.';
