...
export function decodeNFT(data: Buffer | Uint8Array): DecodedNFTMetadata {
  ...
  const name = Buffer.from(data.slice(1 + 32 + 32, 1 + 32 + 32 + 32)).toString('utf8').replace(/\0+$/, '');
  const symbol = Buffer.from(data.slice(1 + 32 + 32 + 32, 1 + 32 + 32 + 32 + 10)).toString('utf8').replace(/\0+$/, '');
  const uri = Buffer.from(data.slice(1 + 32 + 32 + 32 + 10, 1 + 32 + 32 + 32 + 10 + 200)).toString('utf8').replace(/\0+$/, '');
  ...
  if (typeof Buffer !== 'undefined' && data instanceof Buffer) {
    ...
    const dv = new DataView((data as Uint8Array).buffer, (data as Uint8Array).byteOffset, (data as Uint8Array).byteLength);
