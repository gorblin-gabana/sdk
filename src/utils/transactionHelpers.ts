  ...
  if (instructionTypes.some((type) => isNftRelatedInstruction(type))) {
    if (
      instructionTypes.some(
        (type) => type.includes("create") || type.includes("mint"),
      )
    ) {
 ...
 */
export function buildSimpleTransactionSummary(simpleInstructions: any[]): {
  type: string;
  description: string;
} {
...
export function extractAccountChanges(
  ...
  const tokenTransfers = simpleInstructions
    .filter(
      (inst) =>
        inst.action?.includes("Transfer") && inst.action?.includes("Token"),
    )
    .map((inst) => ({
