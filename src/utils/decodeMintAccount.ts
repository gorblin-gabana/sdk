...
export function decodeMintAccount(
  ...
  if (typeof input === 'string') {
    if (opts?.encoding === 'base64') {
      ...
      buf = Uint8Array.from(input.match(/.{1,2}/g)!.map(byte => parseInt(byte, 16)));
    } else {
  ...
  if (buf.length > 82 && buf[82] === 6) {
    ...
    const uri = new TextDecoder().decode(buf.slice(metaStart + 44, metaStart + 200)).replace(/\0+$/, '');
    const updateAuthority = Buffer.from(buf.slice(metaStart + 200, metaStart + 232)).toString('hex');
