  ...
  constructor(
    ...
    super(
      ...
      context,
      {
        ...options,
        retryable: true,
        solution: 'Check your internet connection and RPC endpoint availability. The request will be retried automatically.'
      }
    );
  }
}
  ...
  constructor(
    ...
    super(
      ...
      context,
      {
        ...options,
        retryable: true,
        solution: 'The RPC server is responding slowly. Consider increasing the timeout or switching to a different RPC endpoint.'
      }
    );
  }
}
...
export class RpcServerError extends SDKError {
  ...
  constructor(
    ...
    const retryable = httpStatus ? httpStatus >= 500 || httpStatus === 429 : false;
    const solution = RpcServerError.getSolution(httpStatus, rpcErrorCode);

    super(
      message,
      'RPC_SERVER_ERROR',
      httpStatus && httpStatus >= 500 ? ErrorSeverity.HIGH : ErrorSeverity.MEDIUM,
  ...
  private static getSolution(httpStatus?: number, rpcErrorCode?: number): string {
    if (httpStatus === 429) {
  ...
  constructor(
    ...
    super(
      ...
      context,
      {
        ...options,
        retryable: false,
        solution: 'The RPC server returned an invalid response format. This may indicate a server issue or API version mismatch.'
      }
    );
  }
}
  ...
  constructor(
    ...
    super(
      ...
      context,
      {
        ...options,
        retryable: true,
        solution: 'Request rate limit exceeded. The SDK will automatically retry with exponential backoff.'
      }
    );
  ...
  constructor(
    ...
    super(
      ...
      { ...context, rpcEndpoint: endpoint },
      {
        ...options,
        retryable: true,
        solution: 'Unable to connect to the RPC endpoint. Check your internet connection and ensure the RPC URL is correct.'
      }
    );
  }
}
