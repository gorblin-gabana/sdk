...
export function createDefaultDecoderRegistry(): DecoderRegistry {
  ...
  const splTokenProgramId = config.programIds?.splToken ?? 'Gorbj8Dp27NkXMQUkeHBSmpf6iQ3yT4b2uVe8kM4s6br';
  registry.register('spl-token', splTokenProgramId, wrapSPLTokenDecoder);

  // Register Token-2022 decoder
  const token2022ProgramId = config.programIds?.token2022 ?? 'G22oYgZ6LnVcy7v8eSNi2xpNk1NcZiPD8CVKSTut7oZ6';
  registry.register('token-2022', token2022ProgramId, wrapToken2022Decoder);

  // Register NFT/Metaplex decoder
  const metaplexProgramId = config.programIds?.metaplex ?? 'GMTAp1moCdGh4TEwFTcCJKeKL3UMEDB6vKpo2uxM9h4s';
  ...
  const ataProgramId = config.programIds?.ata ?? 'GoATGVNeSXerFerPqTJ8hcED1msPWHHLxao2vwBYqowm';
