...
describe("Security and Edge Case Testing", () => {
  ...
  describe("Input Validation and Boundary Tests", () => {
    test("Empty and null data handling", async () => {
      ...
      const emptyDecrypted = await decryptPersonalString(
        emptyEncrypted,
        alice.secretKey,
      );
      ...
      const whitespaceEncrypted = await encryptPersonal(
        whitespaceData,
        alice.secretKey,
      );
      const whitespaceDecrypted = await decryptPersonalString(
        whitespaceEncrypted,
        alice.secretKey,
      );
      ...
      const singleCharDecrypted = await decryptPersonalString(
        singleCharEncrypted,
        alice.secretKey,
      );
    ...
    test("Large data boundary testing", async () => {
      ...
      for (const size of sizes) {
        ...
        const encrypted = await encryptPersonal(largeData, alice.secretKey, {
          compress: true,
        });
  ...
  describe("Cryptographic Security Tests", () => {
    test("Key isolation - wrong keys cannot decrypt", async () => {
      ...
      const aliceEncrypted = await encryptPersonal(
        sensitiveData,
        alice.secretKey,
      );
    ...
    test("Ephemeral key uniqueness in direct encryption", async () => {
      ...
      const ephemeralKeys = encryptionResults.map(
        (r) => r.metadata.ephemeralPublicKey,
      );
    ...
    test("Signature forgery resistance", async () => {
      ...
      expect(
        verifySignature(message, aliceSignature, alice.publicKey.toBase58()),
      ).toBe(true);
      ...
      expect(
        verifySignature(message, aliceSignature, bob.publicKey.toBase58()),
      ).toBe(false);

      // Bob cannot forge Alice's signature
      const bobSignature = signData(message, bob.secretKey);
      expect(
        verifySignature(message, bobSignature, alice.publicKey.toBase58()),
      ).toBe(false);
    ...
    test("Metadata tampering detection", async () => {
      ...
      for (let i = 0; i < tamperedVersions.length; i++) {
        ...
        try {
          const result = await decryptPersonal(
            tampered as any,
            alice.secretKey,
          );
          throw new Error(
            `Tampering test ${testNames[i]} failed to detect tampering - decryption succeeded`,
          );
        } catch (error) {
  ...
  describe("Group Security and Access Control Tests", () => {
    ...
    test("Role-based permission enforcement", async () => {
      ...
      const hierarchyGroup = await createSignatureGroup(
        ...
        [
          ...
          {
            publicKey: maliciousUser.publicKey.toBase58(),
    ...
    test("Post-removal access prevention", async () => {
      ...
      expect(Buffer.from(bobDecryptedBefore).toString()).toBe(
        beforeRemovalMessage,
      );
      ...
      expect(
        updatedGroup.members.find(
          (m) => m.publicKey === bob.publicKey.toBase58(),
        ),
      ).toBeUndefined();
  ...
  describe("Shared Key Security Tests", () => {
    test("Permission boundary enforcement", async () => {
      ...
      const charlieKeyShare = sharedKey.encryptedShares.get(
        charlie.publicKey.toBase58(),
      );
  ...
  describe("Error Handling and Recovery Tests", () => {
      ...
      const corruptionTests = [
        {
          ...
          corrupt: (data: any) => ({
            ...data,
            encryptedData: data.encryptedData.slice(0, -5) + "XXXXX",
          }),
        },
        {
          ...
          corrupt: (data: any) => ({
            ...data,
            metadata: { ...data.metadata, nonce: "corrupted_nonce" },
          }),
        },
        {
          name: "Missing method",
    ...
    test("Memory pressure handling", async () => {
      ...
      try {
        ...
        console.log(
          `Successfully handled ${largeDataSets.length} large datasets under memory pressure`,
        );
      } catch (error) {
        // If we hit genuine memory limits, that's expected behavior
        console.log(
          "Memory pressure test hit limits as expected:",
          error.message,
        );
  ...
  describe("Timing Attack Resistance", () => {
    test("Constant-time signature verification", async () => {
      ...
      console.log(
        `Average correct verification time: ${avgCorrectTime.toFixed(3)}ms`,
      );
      console.log(
        `Average incorrect verification time: ${avgIncorrectTime.toFixed(3)}ms`,
      );
      console.log(
        `Timing difference: ${Math.abs(avgCorrectTime - avgIncorrectTime).toFixed(3)}ms`,
      );

      // Timing difference should be minimal (indicating constant-time operation)
      const timingDifferencePercent =
        (Math.abs(avgCorrectTime - avgIncorrectTime) / avgCorrectTime) * 100;
