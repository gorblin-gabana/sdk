...
describe('Enhanced RPC Client V2 Tests', () => {
  ...
  beforeEach(() => {
    ...
    enhancedClient = new EnhancedRpcClient('https://rpc.gorbchain.xyz', baseRpcClient);
  });
  ...
  describe('Method Support Detection', () => {
    ...
    test('should handle unsupported method checks gracefully', async () => {
      const unsupportedMethod = await enhancedClient.isMethodSupported('nonExistentMethod');
    ...
    test('should get token account info with fallback', async () => {
      ...
      try {
        ...
        if (tokenAccounts.length > 0) {
          ...
          const tokenAccountInfo = await enhancedClient.getTokenAccountInfo(tokenAccountAddress);
  ...
  describe('Program Account Operations', () => {
    test('should get program accounts with filters', async () => {
      try {
        ...
        console.log('getProgramAccounts not supported:', (error as Error).message);
      }
    }, 15000);
    ...
    test('should get token accounts by program', async () => {
      try {
        ...
        console.log('Token accounts by program not supported:', (error as Error).message);
      }
    }, 15000);
  });
  ...
  describe('Data Parsing', () => {
    test('should parse token account data correctly', () => {
      ...
      Buffer.from(mockMint.padEnd(44, '1').slice(0, 32)).copy(mockTokenAccountData, 0);
      
      // Set owner (32 bytes starting at offset 32)
      Buffer.from(testWallet.padEnd(44, '1').slice(0, 32)).copy(mockTokenAccountData, 32);
      ...
      try {
        ...
        console.log('Token account parsing failed (expected with mock data):', (error as Error).message);
      }
    });
    ...
    test('should parse mint account data correctly', () => {
      ...
      Buffer.from(testWallet.padEnd(44, '1').slice(0, 32)).copy(mockMintData, 4);
      ...
      try {
        ...
        console.log('Mint account parsing failed (expected with mock data):', (error as Error).message);
      }
    });
  });
  ...
  describe('Custom Token Holdings', () => {
    ...
    test('should handle empty token holdings gracefully', async () => {
      ...
      const holdings = await enhancedClient.getCustomTokenHoldings(emptyWallet, {
        customPrograms: ['TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA']
      });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle network timeouts gracefully', async () => {
      ...
      const timeoutClient = new EnhancedRpcClient('https://nonexistent.rpc.url', timeoutRpcClient);
      ...
      await expect(timeoutClient.isMethodSupported('getAccountInfo')).resolves.toBe(false);
    }, 10000);
