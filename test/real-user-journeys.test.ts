...
describe("Real User Journey Tests", () => {
  ...
  describe("Journey 1: NFT Collector Workflow", () => {
    test("Complete NFT analysis workflow", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping NFT collector journey - requires network and real data",
        );
        return;
      }
      ...
      if (categorized.nfts.length > 0) {
        ...
        console.log(
          `   🔍 Analyzing NFT: ${(firstNFT as any).mint || (firstNFT as any).address || "Unknown"}`,
        );
    ...
    test("NFT metadata resolution workflow", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping NFT metadata test - requires network and real data",
        );
        return;
      }
  ...
  describe("Journey 2: DeFi Portfolio Manager Workflow", () => {
    test("Complete portfolio analysis workflow", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping DeFi portfolio journey - requires network and real data",
        );
        return;
      }
    ...
    test("Portfolio comparison workflow", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping portfolio comparison test - requires network and real data",
        );
        return;
      }
      ...
      if (similarity > 0.7) {
        ...
        console.log(
          "   🎯 Portfolios are quite different - diverse strategies",
        );
      }
    });
  });
  ...
  describe("Journey 3: Transaction Analyst Workflow", () => {
    test("Transaction decoding and analysis workflow", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping transaction analysis journey - requires network and real data",
        );
        return;
      }
      ...
      if (!txSignature) {
        console.log(
          "   ⚠️  No real transaction signatures provided - testing with simulation",
        );
      ...
      if (decodedTx && decodedTx.decoded) {
        ...
        const instructionTypes = decodedTx.decoded.map(
          (ix: any) => ix.type || "unknown",
        );
    ...
    test("Bulk instruction decoding workflow", async () => {
      ...
      const instructions = [
        {
          ...
          data: new Uint8Array([
            2, 0, 0, 0, 0, 0, 0, 0, 64, 66, 15, 0, 0, 0, 0, 0,
          ]),
      ...
      decodedInstructions.forEach((decoded, index) => {
        ...
        console.log(
          `   ${index + 1}. ${decoded.type || "unknown"} (${decoded.programId})`,
        );
      });
  ...
  describe("Journey 4: RPC Integration Tests", () => {
    test("should validate real transaction signatures", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping RPC integration test - requires network access",
        );
        return;
      }
      ...
      if (realTxSignatures.length === 0) {
        console.log(
          "⚠️  No real transaction signatures provided - skipping validation",
        );
        return;
      }

      console.log(
        `🔍 Testing ${realTxSignatures.length} real transaction signatures...`,
      );
      ...
      for (const txSignature of realTxSignatures) {
        ...
        console.log(
          `   ✅ Valid signature format: ${txSignature.substring(0, 20)}...`,
        );
      }
    });
    ...
    test("should handle network errors gracefully", async () => {
      if (skipNetworkTests) {
        console.log(
          "⏭️  Skipping network error test - requires network access",
        );
        return;
      }
  ...
  describe("Journey 5: Network Health Monitor Workflow", () => {
    test("Complete network monitoring workflow", async () => {
      if (skipNetworkTests) {
        console.log(
          "⏭️  Skipping network monitoring journey - requires network access",
        );
        return;
      }
      ...
      console.log(`   📊 Network Stats:`, {
        ...
        version: stats.version
          ? stats.version.solanaCore || "Unknown"
          : "Unknown",
    ...
    test("Network resilience testing", async () => {
      if (skipNetworkTests) {
        console.log(
          "⏭️  Skipping network resilience test - requires network access",
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `   ⚠️  Network stress test triggered timeout (expected behavior)`,
        );
