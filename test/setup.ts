...
const TEST_DATA = {
  wallets: {
    diverse:
      process.env.TEST_WALLET_DIVERSE ||
      "7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU",
    nft:
      process.env.TEST_WALLET_NFT ||
      "5YNmS1R9nNSCDzAYUGd2xgXzUk8A5mWJPybyGCPP6gFW",
    token:
      process.env.TEST_WALLET_TOKEN ||
      "7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU",
    empty:
      process.env.TEST_WALLET_EMPTY ||
      "5YNmS1R9nNSCDzAYUGd2xgXzUk8A5mWJPybyGCPP6gFW",
  },
  ...
  nfts: {
    mplCore: {
      address:
        process.env.TEST_NFT_MPL_CORE ||
        "FWsWxqEjpy6B2Je5TwhUsB3aAqXHPgwApWEoHnmnsorZ",
      owner:
        process.env.TEST_NFT_OWNER ||
        "7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU",
    },
  },
...
beforeAll(() => {
  ...
  if (!process.env.TEST_TX_TOKEN_TRANSFER) {
    issues.push(
      "TEST_TX_TOKEN_TRANSFER not provided - transaction tests may be limited",
    );
  }
  ...
  if (issues.length > 0 && process.env.VERBOSE_TESTS === "true") {
    ...
    console.warn(
      "   See TESTING_STRATEGY.md for instructions on providing real test data",
    );
  }
});
