...
describe('Performance and Stress Testing', () => {
  ...
  describe('Encryption Performance Benchmarks', () => {
    test('Personal encryption performance with various data sizes', async () => {
      ...
      for (const testCase of testSizes) {
        ...
        const encrypted = await encryptPersonal(testCase.data, alice.secretKey, { 
          compress: true 
        })
        ...
        const result = {
          ...
          encryptTime,
          decryptTime,
          throughputMBps: (testCase.data.length / 1024 / 1024) / (encryptTime / 1000)
        }

        results.push(result)

        // Performance thresholds
        if (testCase.data.length <= 10 * 1024) { // <= 10KB
          expect(encryptTime).toBeLessThan(100) // Should encrypt in under 100ms
          expect(decryptTime).toBeLessThan(100)
        } else if (testCase.data.length <= 100 * 1024) { // <= 100KB
    ...
    test('Direct encryption performance scaling', async () => {
      ...
      // Warm up
      for (let i = 0; i < 5; i++) {
        const encrypted = await encryptDirect(message, bob.publicKey.toBase58(), alice.secretKey)
      ...
      const avgEncryptTime = encryptTimes.reduce((a, b) => a + b) / encryptTimes.length
      const avgDecryptTime = decryptTimes.reduce((a, b) => a + b) / decryptTimes.length
      ...
      console.log(`  Operations per second: ${(1000 / avgEncryptTime).toFixed(0)} ops/s`)
    ...
    test('Signature verification performance', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const isValid = verifySignature(message, signature, alice.publicKey.toBase58())
      ...
      const avgVerifyTime = verificationTimes.reduce((a, b) => a + b) / verificationTimes.length
      ...
      console.log(`  Verifications per second: ${(1000 / avgVerifyTime).toFixed(0)} ops/s`)
  ...
  describe('Scalability Stress Tests', () => {
    test('Large group creation and message encryption stress test', async () => {
      ...
      const testMessage = 'Broadcasting to large group - performance test message'
      ...
      const decryptionPromises = testUsers.slice(1, 11).map(async (user, index) => {
        const start = performance.now()
      ...
      console.log(`  Parallel decryption time (10 members): ${totalParallelTime.toFixed(2)}ms`)
      ...
      const avgDecryptTime = decryptResults.reduce((sum, r) => sum + r.time, 0) / decryptResults.length
      console.log(`  Average individual decrypt time: ${avgDecryptTime.toFixed(2)}ms`)
    ...
    test('Scalable encryption context stress test - rapid scaling', async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        const newRecipients = testUsers.slice(startIndex, startIndex + batchSize)
        ...
        console.log(`  Sample decryption time (${sampleSize} users): ${decryptTime.toFixed(2)}ms`)
    ...
    test('Shared key manager stress test - many concurrent operations', async () => {
      ...
      console.log(`  Total concurrent key creation time: ${totalKeyCreationTime.toFixed(2)}ms`)
      console.log(`  Average time per key: ${(totalKeyCreationTime / numKeys).toFixed(2)}ms`)
      ...
      console.log(`  Total concurrent encryption time: ${totalEncryptionTime.toFixed(2)}ms`)
      console.log(`  Average time per encryption: ${(totalEncryptionTime / numKeys).toFixed(2)}ms`)
      ...
      console.log(`  Total concurrent decryption time: ${totalDecryptionTime.toFixed(2)}ms`)
      console.log(`  Average time per decryption: ${(totalDecryptionTime / numKeys).toFixed(2)}ms`)
  ...
  describe('Memory and Resource Usage Tests', () => {
    test('Memory usage during large group operations', async () => {
      ...
      const largeGroup = await createSignatureGroup(
        'Memory Test Group',
        creator.secretKey,
        testUsers.slice(1, 31).map(user => ({ // 30 members
          publicKey: user.publicKey.toBase58(),
          role: MemberRole.MEMBER
        }))
      )
      ...
      if (initialMemory && afterGroupMemory && afterEncryptionMemory) {
        ...
        console.log(`  After group creation: ${(afterGroupMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`)
        console.log(`  After 50 encryptions: ${(afterEncryptionMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`)
        
        if (afterGCMemory) {
          console.log(`  After GC: ${(afterGCMemory.heapUsed / 1024 / 1024).toFixed(2)} MB`)
        }
        
        const groupMemoryIncrease = (afterGroupMemory.heapUsed - initialMemory.heapUsed) / 1024 / 1024
        const encryptionMemoryIncrease = (afterEncryptionMemory.heapUsed - afterGroupMemory.heapUsed) / 1024 / 1024
        
        console.log(`  Group creation memory increase: ${groupMemoryIncrease.toFixed(2)} MB`)
        console.log(`  Encryption operations memory increase: ${encryptionMemoryIncrease.toFixed(2)} MB`)
    ...
    test('Resource cleanup after operations', async () => {
      ...
      for (let i = 0; i < operations; i++) {
        ...
        const personalEncrypted = await encryptPersonal(`Personal message ${i}`, alice.secretKey)
        const personalDecrypted = await decryptPersonalString(personalEncrypted, alice.secretKey)
        expect(personalDecrypted).toBe(`Personal message ${i}`)

        // Direct encryption/decryption
        const directEncrypted = await encryptDirect(`Direct message ${i}`, bob.publicKey.toBase58(), alice.secretKey)
        const directDecrypted = await decryptDirectString(directEncrypted, bob.secretKey)
        expect(directDecrypted).toBe(`Direct message ${i}`)

        // Signature operations
        const signature = signData(`Sign message ${i}`, alice.secretKey)
        const isValid = verifySignature(`Sign message ${i}`, signature, alice.publicKey.toBase58())
        expect(isValid).toBe(true)
        ...
        if (i > 0 && i % 25 === 0) {
          ...
          if (memUsage) {
            console.log(`  After ${i} operations: ${(memUsage.heapUsed / 1024 / 1024).toFixed(2)} MB heap used`)
          }
        }
      }
  ...
  describe('Concurrent Operations Stress Test', () => {
    test('High concurrency mixed operations', async () => {
      ...
      for (let i = 0; i < numConcurrentOps; i++) {
        ...
        switch (i % 4) {
          case 0: // Personal encryption
            promise = (async () => {
              const encrypted = await encryptPersonal(`${testMessage} ${i}`, userA.secretKey)
              const decrypted = await decryptPersonalString(encrypted, userA.secretKey)
              return { type: 'personal', success: decrypted === `${testMessage} ${i}` }
            })()
            break

          case 1: // Direct encryption
            promise = (async () => {
              const encrypted = await encryptDirect(`${testMessage} ${i}`, userB.publicKey.toBase58(), userA.secretKey)
              const decrypted = await decryptDirectString(encrypted, userB.secretKey)
              return { type: 'direct', success: decrypted === `${testMessage} ${i}` }
            })()
            break

          case 2: // Signature operations
            promise = (async () => {
              const signature = signData(`${testMessage} ${i}`, userA.secretKey)
              const isValid = verifySignature(`${testMessage} ${i}`, signature, userA.publicKey.toBase58())
          ...
          case 3: // Group operations
            promise = (async () => {
              const group = await createSignatureGroup(
                `Concurrent Group ${i}`,
                userA.secretKey,
                [{ publicKey: userB.publicKey.toBase58(), role: MemberRole.MEMBER }]
              )
              const encrypted = await encryptForSignatureGroup(
                `${testMessage} ${i}`,
                group,
              ...
              const decrypted = await decryptSignatureGroupData(encrypted, userA.secretKey, userA.publicKey.toBase58())
              return { type: 'group', success: Buffer.from(decrypted).toString() === `${testMessage} ${i}` }
            })()
            break
        }
      ...
      console.log(`  Total concurrent operations time: ${totalTime.toFixed(2)}ms`)
      console.log(`  Average time per operation: ${(totalTime / numConcurrentOps).toFixed(2)}ms`)
      console.log(`  Operations per second: ${(numConcurrentOps / (totalTime / 1000)).toFixed(0)} ops/s`)
      ...
      Object.entries(resultsByType).forEach(([type, successes]) => {
        ...
        console.log(`  ${type}: ${successCount}/${successes.length} successful`)
