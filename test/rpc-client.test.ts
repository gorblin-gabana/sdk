...
describe("RpcClient Tests", () => {
  ...
  describe("Client Configuration", () => {
    test("should use gorbchain config by default", () => {
      ...
      expect(defaultClient.getRpcUrl()).toBe(
        config.rpcUrl || "https://rpc.gorbchain.xyz",
      );
    });
  ...
  describe("RPC Request Handling", () => {
    ...
    test("should handle RPC errors", async () => {
      ...
      await expect(client.request("getSlot", ["invalid"])).rejects.toThrow(
        /Invalid params/,
      );
    });
    ...
    test("should handle HTTP errors", async () => {
      ...
      await expect(client.request("getSlot")).rejects.toThrow(
    ...
    test("should not retry on 4xx errors", async () => {
      ...
      await expect(noRetryClient.request("invalidMethod")).rejects.toThrow();
    ...
    test("should handle timeout", async () => {
      ...
      await expect(shortTimeoutClient.request("getSlot")).rejects.toThrow();
    });
  });
