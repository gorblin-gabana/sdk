...
describe("Real-World Messaging Scenarios", () => {
  ...
  describe("Private Personal Notes & Drafts", () => {
    ...
    test("Personal conversation history encryption", async () => {
        ...
        messages: [
          {
            ...
            content: "Hey Bob, can you help with the API?",
            timestamp: Date.now() - 3600000,
          },
          {
            sender: bob.publicKey.toBase58(),
            content: "Sure! I'll take a look this afternoon",
            timestamp: Date.now() - 3000000,
          },
          {
            sender: alice.publicKey.toBase58(),
            content: "Thanks! The endpoint is /api/users",
      ...
      expect(recoveredHistory.participants).toEqual(
        conversationHistory.participants,
      );
      expect(recoveredHistory.messages).toHaveLength(3);
      expect(recoveredHistory.messages[0].content).toBe(
        "Hey Bob, can you help with the API?",
      );
    });
  });
  ...
  describe("Direct Messaging (1-on-1 Chat)", () => {
    test("Basic secure messaging between Alice and Bob", async () => {
      ...
      expect(encrypted.metadata.senderPublicKey).toBe(
        alice.publicKey.toBase58(),
      );
      expect(encrypted.metadata.recipientPublicKey).toBe(
        bob.publicKey.toBase58(),
      );
    ...
    test("Confidential HR conversation", async () => {
      ...
      const hrMessage = {
        ...
        content:
          "Alice, here are the salary adjustment details for your team members...",
        salaryAdjustments: [
  ...
  describe("Group Messaging - Small Team Chat", () => {
    test("Development team group chat", async () => {
      ...
      const announcement =
        "Sprint planning meeting tomorrow at 10 AM. Please review the tickets in advance.";
    ...
    test("Adding new team member to existing conversation", async () => {
      ...
      const initialMessage =
        "Welcome to Project Alpha! We'll be working on the new authentication system.";
      ...
      const updateMessage =
        "Diana has joined the team! She'll help with requirements gathering.";
    ...
    test("Removing team member with key rotation for security", async () => {
      ...
      expect(
        updatedTeam.members.find(
          (m) => m.publicKey === eve.publicKey.toBase58(),
        ),
      ).toBeUndefined();

      // Send sensitive message after contractor removal
      const sensitiveMessage =
        "Now that the contractor is gone, let's discuss the budget constraints...";
      const encrypted2 = await encryptForSignatureGroup(
        sensitiveMessage,
        updatedTeam,
        alice.secretKey,
  ...
  describe("Scalable Group Messaging - Growing Teams", () => {
    test("Startup team growth - auto-scaling encryption", async () => {
      ...
      const growthMessage =
        "Team update: We're now 4 people! Let's plan our next quarter.";
    ...
    test("Department reorganization - member transitions", async () => {
      ...
      const campaignMessage =
        "Q1 campaign budget approved: $50k. Let's plan the social media strategy.";
      const encrypted1 = await manager.encryptInContext(
        context.contextId,
        campaignMessage,
        bob.secretKey,
      ...
      const newCampaignMessage =
        "Welcome Eve! Let's discuss the new product launch strategy.";
      const encrypted2 = await manager.encryptInContext(
        context.contextId,
        newCampaignMessage,
        bob.secretKey,
  ...
  describe("Message Signing and Verification", () => {
    test("Important announcements with signature verification", async () => {
      ...
      const devTeam = await createSignatureGroup("All Hands", alice.secretKey, [
        { publicKey: bob.publicKey.toBase58(), role: MemberRole.MEMBER },
        { publicKey: charlie.publicKey.toBase58(), role: MemberRole.MEMBER },
    ...
    test("Legal document approval workflow", async () => {
      ...
      const fullySignedContract = {
        ...
        signatures: [
          {
            signer: alice.publicKey.toBase58(),
            signature: encodeBase58(aliceSignature),
            role: "CEO",
            timestamp: Date.now(),
          },
          {
            signer: bob.publicKey.toBase58(),
            signature: encodeBase58(bobSignature),
            role: "CTO",
            timestamp: Date.now() + 1000,
          },
          {
            signer: eve.publicKey.toBase58(),
            signature: encodeBase58(eveSignature),
            role: "Contractor",
      ...
      const recoveredContract = JSON.parse(
        Buffer.from(eveDecrypted).toString(),
      );
  ...
  describe("Message Threading and Conversation Management", () => {
    test("Threaded conversations with encrypted replies", async () => {
      ...
      const decryptedOriginal = JSON.parse(
        Buffer.from(
          await decryptSignatureGroupData(
            encryptedOriginal,
            alice.secretKey,
            alice.publicKey.toBase58(),
          ),
      ...
      const decryptedBobReply = JSON.parse(
        Buffer.from(
          await decryptSignatureGroupData(
            encryptedBobReply,
            alice.secretKey,
      ...
      const decryptedCharlieReply = JSON.parse(
        Buffer.from(
          await decryptSignatureGroupData(
            encryptedCharlieReply,
            alice.secretKey,
            alice.publicKey.toBase58(),
          ),
        ).toString(),
  ...
  describe("Emergency and Security Scenarios", () => {
    test("Emergency key rotation after security incident", async () => {
      ...
      const emergencyMessage =
        "SECURITY ALERT: Suspicious activity detected. All keys being rotated.";
      ...
      const postIncidentMessage =
        "Security incident resolved. New keys distributed. All clear.";
