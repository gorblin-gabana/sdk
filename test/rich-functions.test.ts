...
describe("Rich Functions Integration Tests", () => {
  ...
  describe("Rich Token Operations", () => {
    test("getRichTokenAccountsByOwner - comprehensive token analysis", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping rich token accounts test - requires network and real data",
        );
        return;
      }
      ...
      console.log(
        `   🎭 Summary: ${richTokens.summary.totalTokens} tokens, ${richTokens.summary.totalNFTs} NFTs`,
      );
      ...
      if (richTokens.accounts.length > 0) {
        ...
        console.log(
          `   ✅ Account structure verified for ${firstAccount.metadata.symbol || "unknown token"}`,
        );
      }
    ...
    test("SDK integration - getRichTokenAccounts method", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping SDK integration test - requires network and real data",
        );
        return;
      }
      ...
      console.log(
        `   ✅ SDK method returned ${result.accounts.length} accounts`,
      );
    });
    ...
    test("Rich token accounts - error handling", async () => {
      ...
      if (!skipNetworkTests && !skipRealDataTests) {
        const result = await getRichTokenAccountsByOwner(
          sdk,
          TEST_DATA.wallets.diverse,
        );
        expect(result).toHaveProperty("accounts");
  ...
  describe("Rich Transaction Operations", () => {
    test("getRichTransaction - comprehensive transaction analysis", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping rich transaction test - requires network and real data",
        );
        return;
      }

      const txSignature =
        TEST_DATA.transactions.tokenTransfer || TEST_DATA.transactions.complex;

      if (!txSignature || txSignature.length === 0) {
        console.log(
          "⚠️  No real transaction signature provided - skipping test",
        );
        return;
      }
      ...
      if (richTx.instructions.length > 0) {
        ...
        console.log(
          `   ✅ Instruction structure verified: ${firstInstruction.description}`,
        );
      }
    ...
    test("SDK integration - getRichTransaction method", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping SDK rich transaction test - requires network and real data",
        );
        return;
      }
      ...
      if (!txSignature) {
        console.log(
          "⚠️  No NFT mint transaction signature provided - skipping test",
        );
        return;
      }
      ...
      expect(result).toHaveProperty("summary");
      console.log(
        `   ✅ SDK method analyzed transaction: ${result.summary.primaryAction}`,
      );
    });
  ...
  describe("Universal Wallet Integration", () => {
    test("UniversalWalletManager - basic functionality", async () => {
      ...
      console.log(
        `   🔍 Discovered ${discovery.available.length} wallet options`,
      );
      ...
      if (discovery.available.length > 0) {
        ...
        console.log(
          `   ✅ Wallet structure verified: ${firstWallet.name} (${firstWallet.installed ? "installed" : "not installed"})`,
        );
      }
    });
    ...
    test("Wallet manager - error handling", async () => {
      ...
      try {
        ...
        console.log(
          "   ⚠️  Phantom connection may have succeeded (wallet might be installed)",
        );
      } catch (error) {
        expect(error).toBeDefined();
        console.log(
          "   ✅ Non-existent wallet connection error handled gracefully",
        );
      }
      ...
      expect(
        autoConnectResult === null || typeof autoConnectResult === "object",
      ).toBe(true);
  ...
  describe("Rich Functions Performance Tests", () => {
    test("Performance benchmarks", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping performance tests - requires network and real data",
        );
        return;
      }
      ...
      console.log(
        `   📊 Rich token accounts: ${tokenTime}ms (${tokenResult.accounts.length} accounts)`,
      );
      ...
      console.log(
        `   👛 Wallet discovery: ${walletTime}ms (${discovery.available.length} wallets)`,
      );
    ...
    test("Concurrent operations stress test", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping stress test - requires network and real data",
        );
        return;
      }
  ...
  describe("Rich Functions Integration with Existing SDK", () => {
    test("Rich functions complement existing functionality", async () => {
      if (skipNetworkTests || skipRealDataTests) {
        console.log(
          "⏭️  Skipping integration test - requires network and real data",
        );
        return;
      }
      ...
      // Compare results
      console.log(
        `   📊 Basic holdings: ${basicHoldings.holdings.length} items`,
      );
      console.log(`   🎯 Rich holdings: ${richHoldings.accounts.length} items`);

      // Rich holdings should provide additional context
      if (
        richHoldings.accounts.length > 0 &&
        basicHoldings.holdings.length > 0
      ) {
        const richAccount = richHoldings.accounts[0];
        const basicAccount = basicHoldings.holdings.find(
          (h) => h.mint === richAccount.mint,
        );
        ...
        if (basicAccount) {
          ...
          console.log(
            `   ✅ Rich account provides enhanced data for ${richAccount.metadata.symbol || "token"}`,
          );
        }
      }
