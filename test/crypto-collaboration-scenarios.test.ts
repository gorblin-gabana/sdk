...
describe('Real-World Collaboration Scenarios', () => {
  ...
  describe('Document Management System', () => {
    ...
    test('Version-controlled document updates with audit trail', async () => {
      ...
      const apiDocsV1 = {
        ...
        changelog: [
          { version: '1.0.0', date: new Date().toISOString(), changes: 'Initial version' }
        ]
      }

      // Alice signs the document
      apiDocsV1.signature = signData(JSON.stringify(apiDocsV1.content), alice.secretKey)
      ...
      apiDocsV11.signature = signData(JSON.stringify(apiDocsV11.content), bob.secretKey)
    ...
    test('Temporary document access for external parties', async () => {
      ...
      const internalDoc = {
        ...
        content: 'Detailed roadmap with internal metrics and confidential initiatives...',
      ...
      const clientAccessKey = await sharedKeyManager.createSharedKey(
        ...
        [
          { publicKey: alice.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: true, canShare: false, canRevoke: true }},
          { publicKey: eve.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: false, canShare: false, canRevoke: false }} // Client read-only
        ],
        alice.secretKey
      )
      ...
      const revokedClientKey = await sharedKeyManager.removeRecipientsFromSharedKey(
        clientAccessKey.keyId,
        [eve.publicKey.toBase58()],
        alice.secretKey,
        alice.publicKey.toBase58(),
        ...
        line_comments: [
          {
            ...
            line: 45,
            comment: 'Using hardcoded secret temporarily - will fix before merge'
          }
        ],
        ...
        line_comments: [
          {
            file: 'src/auth/oauth.js',
            line: 45,
            comment: 'CRITICAL: Remove hardcoded secret before merge. Use process.env.OAUTH_SECRET',
            type: 'security'
          }
        ],
        overall_comment: 'Good implementation, but security issues must be addressed before merge.',
    ...
    test('API documentation with encrypted implementation details', async () => {
      ...
      const techDocsKey = await sharedKeyManager.createSharedKey(
        ...
        [
          { publicKey: alice.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: true, canShare: true, canRevoke: true }},
          { publicKey: bob.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: true, canShare: false, canRevoke: false }},
          { publicKey: charlie.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: false, canShare: false, canRevoke: false }} // Designer read-only
        ],
        alice.secretKey
      )
      ...
      expect(bobTechDoc.implementation_details.api_keys.live).toContain('sk_live_xyz789')
      ...
      const charlieTechDoc = JSON.parse(Buffer.from(charlieDecrypted).toString())
      ...
      const charliePermissions = techDocsKey.holders.find(h => h.publicKey === charlie.publicKey.toBase58())
  ...
  describe('Legal and Compliance Documentation', () => {
    test('Multi-party contract negotiation with encrypted drafts', async () => {
      ...
      expect(aliceDraft.confidential_notes).toContain('Competitor bid was $45,000')
      ...
      const businessInput = {
        ...
        comments: [
          {
            clause: 'liability_cap',
            business_comment: 'We can increase to $250,000 if client pays premium',
        ...
        confidential_strategy: 'Focus on multi-year commitment over per-year pricing',
        approval_limits: {
      ...
      const businessFeedback = JSON.parse(Buffer.from(dianaDecryptedInput).toString())
    ...
    test('Compliance audit trail with encrypted audit logs', async () => {
      ...
      const auditKey = await sharedKeyManager.createSharedKey(
        ...
        [
          { publicKey: alice.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: true, canShare: true, canRevoke: true }},
          { publicKey: diana.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: true, canShare: false, canRevoke: false }}, // Legal
          { publicKey: frank.publicKey.toBase58(), permissions: { canDecrypt: true, canEncrypt: false, canShare: false, canRevoke: false }} // External auditor
        ],
        alice.secretKey
      )
      ...
      const auditEvidence = {
        ...
        findings: [
          ...
          },
          {
            ...
            sensitive_finding: 'One legacy system using TLS 1.2 - remediation scheduled',
      ...
      expect(frankAudit.confidential_observations).toContain('excessive database privileges')
  ...
  describe('Performance and Scalability Testing', () => {
    test('Large document encryption performance', async () => {
      ...
      const largeContent = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. '.repeat(20000) // ~1MB
    ...
    test('Many recipients performance test', async () => {
      ...
      const testDocument = {
        ...
        date: new Date().toISOString(),
        content: 'Meeting notes with financial updates and strategic direction...',
      ...
      const decryptPromises = manyRecipients.slice(0, 5).map(async recipient => {
      ...
      console.log(`  Encryption time: ${encryptTime}ms`)
      console.log(`  Parallel decryption time (5 recipients): ${decryptTime}ms`)
